...
class EnemyMinelayer extends Enemy {
    ...
    static activityDefnPerform(uwpe) {
        ...
        if (enemySpeed == 0) {
            ...
            var enemyVelToSet = Coords.oneOneZero().normalize().multiplyScalar(enemySpeedFixed);
            enemyVel.overwriteWith(enemyVelToSet);
        }
        var enemyPos = enemyDisp.pos;
        var enemyPosY = enemyPos.y;
        var enemyPosYIsOutsideAllowedRange = enemyPosY < altitudesToFlyBetween[0]
    ...
    static activityDefnPerform_FireMineLauncherIfCharged(uwpe) {
        var enemy = uwpe.entity;
        var place = uwpe.place;
    ...
    static deviceMineLauncherBuild() {
        return Device.fromNameTicksToChargeAndUse("Gun", 20, // 1 seconds
        // 1 seconds
    ...
    static projectileShooterBuild() {
        ...
        var radius = 6;
        var projectileVisual = visualBuilder.starburstWithPointsRatioRadiusAndColor(4, .25, radius, projectileColor);
        var init = (entity) => PlacePlanet.projectileShooterBuild_CollidableConstrainableAndDrawableWrapForEntity(entity);
        var projectileGeneration = new ProjectileGeneration(radius, 0, // distanceInitial
        0, // speed
        100, // ticksToLive - 5 seconds
        null, // integrityMax
        propertiesToCollideWithNames, null, // damage
        projectileVisual, init, null // hit
        );
        var projectileGenerator = ProjectileGenerator.fromGeneration(projectileGeneration);
        var shooter = ProjectileShooter.fromNameAndGenerator(EnemyMinelayer.name + ProjectileShooter.name, projectileGenerator);
    ...
    static visualBuild() {
        ...
        var visualShaft = VisualRectangle.fromSizeAndColorFill(Coords.fromXY(1.5, 1).multiplyScalar(dimension), colorBody);
        var dimensionLauncher = dimension * 1.2;
        var visualLauncherFrame = VisualRectangle.fromSizeAndColorFill(Coords.ones().multiplyScalar(dimensionLauncher), colorBody);
        var launcherPortRadius = dimensionLauncher * 0.4;
        var visualLauncherPort = VisualCircle.fromRadiusAndColorFill(launcherPortRadius, colorLauncherPort);
        var visualLauncher = VisualGroup.fromChildren([
            visualLauncherFrame, visualLauncherPort
        ]);
        var visualLauncherOffsetLeft = VisualOffset.fromOffsetAndChild(Coords.fromXY(-1, 0).multiplyScalar(dimension), visualLauncher);
        var visualLauncherOffsetRight = VisualOffset.fromOffsetAndChild(Coords.fromXY(1, 0).multiplyScalar(dimension), visualLauncher);
